...
public class C_Arena_PvP extends GameObject
{
	...
	public static boolean[]   P2T1= {false,false,false};
	public static boolean[][] P2T2= {{false,false},{false,false},{false,false}};
	...
	public void update(GameEngine ge, float dt)
	{
		if(p1.isMyTurn())
		{
			if(Scripts.isClicked(attack,ax,ay))	 {P1T1[0]=true;P1T1[1]=false;P1T1[2]=false;}
			if(Scripts.isClicked(parry,px,py))	 {P1T1[0]=false;P1T1[1]=true;P1T1[2]=false;}
			if(Scripts.isClicked(heal,hx,hy))	 {P1T1[0]=false;P1T1[1]=false;P1T1[2]=true;}
		...
		if(p2.isMyTurn())
		{
			if(Scripts.isClicked(attack,ax,ay))	 {P2T1[0]=true;P2T1[1]=false;P2T1[2]=false;}
			if(Scripts.isClicked(parry,px,py))	 {P2T1[0]=false;P2T1[1]=true;P2T1[2]=false;}
			if(Scripts.isClicked(heal,hx,hy))	 {P2T1[0]=false;P2T1[1]=false;P2T1[2]=true;}
			for(int i=0;i<P2T1.length;i++)
			{
				if (P2T1[i])
				{
					weaponList.clear();
					for(Weapon w:weapon2[i])
					{
						if(w!=null) {weaponList.add(w.getImage());}
					}
					Py=375+50*i;
			...
			for(int k=0;k<3;k++)
			{
				if(P2T1[k])
				{
					...
					{
						Iy=375+50*i;
						if(Scripts.isHovered(weaponList.get(i),Ix, Iy)) {P2T2[k][i]=true;}
						else {P2T2[k][i]=false;}
					}
				}
			}
			if(Scripts.isTrueInList(P2T2))	{additionalInfo=true;}
			else 							{additionalInfo=false;}
		}
	...
	public void render(GameEngine ge, Renderer r)
	{
		...
		if(additionalInfo)
		{
			...
			if(p2.isMyTurn())
			{
				r.drawNumber((int)Player_Right.playerAction[0], 2845, 405);
				r.drawNumber((int)Player_Right.playerAction[1], 2845, 455);
			}
		}
